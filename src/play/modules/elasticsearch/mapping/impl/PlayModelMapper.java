...
public class PlayModelMapper<M extends Model> implements ModelMapper<M> {
    ...
    public PlayModelMapper(Class<M> clazz) {
        ...
        if(meta == null || StringUtils.isEmpty(meta.indexName())) {
            String[] tokenizedName = StringUtils.split(clazz.getName().trim().toLowerCase(), ".");
            if(tokenizedName != null && tokenizedName.length > 1){
                // If we have a pkg name use this for index
                indexNameFromMeta = StringUtils.join(tokenizedName, "_", 0, tokenizedName.length-1);
            }else{
                indexNameFromMeta = getTypeName();
            }
        } else {
            indexNameFromMeta = meta.indexName();
        }
        
        if(meta == null){
            riverSQL = "";
        }else{
            riverSQL = meta.riverSQL();
        }
